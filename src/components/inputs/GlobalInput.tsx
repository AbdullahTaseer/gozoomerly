  ...
  return (
    <div className={`space-y-1 w-full ${className ?? ""}`}>
      ...
      <input
        ...
        className={`rounded-md placeholder:text-[15px] text-[15px] text-black autofill:text-[#A6A6A6] placeholder:text-[#020202] border focus:outline-none block placeholder:font-[300] border-[#2E2C39] px-4 ${inputClassName ?? ""}`}
      />
      {(error || localError) && (
        <div className={`${error || localError ? "mb-1" : ""}`}>
          <span className="text-sm text-red-500 px-1">{error || localError}</span>
